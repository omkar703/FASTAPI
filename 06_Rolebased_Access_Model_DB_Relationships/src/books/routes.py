...
@router_book.patch("/{book_id}", response_model=Book , status_code=status.HTTP_200_OK , dependencies=[Depends(access_token_bearer)])
async def update_a_book(book_id: str, book_update_data:  UpdateBookModel, session: AsyncSession = Depends(get_session),dependencies=[Depends(access_token_bearer)]) -> Book:
